//**조건부 코드는 매크로로 정의되어 있을때만 컴파일 되는 코드입니다**
//라이브러리 포함
#include <avr/io.h>
#include <Wire.h>
#include "config.h"
...
//pid제어 이름 매크로 정리 -위에서 정의한 이름을 간략화
...
//********중요!!!!드론 제어 변수 정의********* - 드론 핵심 데이터 저장/인터페이스에 활용 가능,각종 드론 제어 계산에 필요
static uint32_t currentTime = 0; //시간을 밀리초로 저장
static uint16_t previousTime = 0;//이전 루프의 시간 저장
static uint16_t cycleTime = 0;     // 한 루프에 필요한 시간을 밀리초로 저장
static uint16_t calibratingA = 0;  // 가속도계 캘리브레이션 상태
static uint16_t calibratingB = 0;  // 고도계 캘리브레이션 상태
static uint16_t calibratingG;      // 자이로 스코프 캘리브레이션 상태
static uint16_t acc_1G;            // 1G 가속도에 해당하는 측정된 가속도 값을 저장(수평상태에서 측정)
static uint16_t acc_25deg;          //25도 기울기에 해당하는 가속도 값을 저장
static int16_t  gyroADC[3],accADC[3],accSmooth[3],magADC[3]; // 자이로 센서 측정값, 가속도계 측정값,가속도 측정 필터값,자기장 센서에 측정값,-각 축별로 저장하므로 [3]이 있음
static int16_t  heading,magHold,headFreeModeHold; // 드론의 현재 방향을 [-180;+180] 범위로 저장,자기장에 기반한 목표 방향을 저장,HEADFREE 모드에서의 기준 방향을 저장
static uint8_t  vbat;                   // 배터리 전압을 0.1V 단위로 저장
static uint8_t  vbatMin = VBATNOMINAL;  // 비행 중 기록된 가장 낮은 배터리 전압을 저장
static uint8_t  rcOptions[CHECKBOXITEMS];//RC 수신기에서 받은 선택된 기능 옵션을 저장
static int32_t  BaroAlt,EstAlt,AltHold; // 바로미터로 측정한 고도를 cm 단위로 저장,추정된 고도를 cm 단위로 저장,고도 유지 모드에서 목표 고도를 cm 단위로 저장합니다.
static int16_t  BaroPID = 0;            //고도 제어를 위한 PID 값 중 P(비례) 성분을 저장
static int16_t  errorAltitudeI = 0;     //고도 유지 제어에서 적분 오차 값을 저장,고도 제어의 I(적분) 성분에 사용
static int16_t  vario = 0;              // 드론의 상승 또는 하강 속도를 cm/s 단위로 저장

//ARMEDTIMEWARNING매크로가 정의 되어 있을때 알람시간을 마이크로 초마다 저장
#if defined(ARMEDTIMEWARNING)
  static uint32_t  ArmedTimeWarningMicroSeconds = 0;
#endif

static int16_t  debug[4];//디버깅 변수 
static int16_t  sonarAlt; //초음파 거리 측정 변수

//-------------------------------------------------------------------------
//0,1을 통해 현재 드론의 상태를 나타내는 변수
uint8_t calibration_flag;
//-------------------------------------------------------------------------

//드론 모드 구조체 :해당하는 모드가 켜져있으면 1.아니면 0
struct flags_struct {
  uint8_t OK_TO_ARM :1 ;
  uint8_t ARMED :1 ;
...
static int16_t rcCommand[4];        //스로틀[1000,2000]범위,요,피치롤은[-500,500]범위
static int16_t lookupPitchRollRC[6];
static int16_t lookupThrottleRC[11];
static uint16_t rssi;               

#if defined(SPEKTRUM)//조건부(중요x)
  volatile uint8_t  spekFrameFlags;
  volatile uint32_t spekTimeLast;
#endif

#if defined(OPENLRSv2MULTI)//(조건부(중요x))
...
// *****중요 자이로 센서 변수!!!!!!!!!*************
// **************
static int16_t gyroData[3] = {0,0,0};//자이로 각속도 측정값을 축별로 저장
static int16_t gyroZero[3] = {0,0,0};//자이로 센서의 편차값을 축별로 저장
static int16_t angle[2]    = {0,0};//롤과 피치값을 저장

// *************************
// **************모터와 서보변수 !!!!**************
// *************************
static int16_t axisPID[3];//롤,피치,요에 대한 pid 제어값 저장
static int16_t motor[NUMBER_MOTOR];//모터에 전달할 출력값 저장
#if defined(SERVO)//서브 모터 조건부 코드(중요x)
...
// 비 휘발성 메모리 변수
// ************************
static uint8_t dynP8[3], dynD8[3];//pid제어 변수
...
#ifdef LOG_PERMANENT//영구로그 조건부 구조체 코드
static struct {
